...
import { ACTIVITIES, LOG_ACTIVITY, UPDATE_ACTIVITY, DELETE_ACTIVITY } from '../graphql/operations/activities'
...
export default function ActivitiesPage() {
  ...
  async function onOk() {
    try {
      ...
      const hide = messageApi.loading(editing ? 'Updating activity...' : 'Logging activity...', 0)
      
      if (editing) {
        await updateActivityMut({ variables: { updateActivityId: editing.id, input: activityInput } })
        hide()
    ...
    },
    {
      ...
      render: (_: any, record: any) => (
        <>
          ...
          <Popconfirm title="Delete activity?" onConfirm={() => onDelete(record.id)}>
    ...
    <div style={{ padding: '24px' }}>
      ...
      <div style={{ marginBottom: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          ...
          <p style={{ color: '#8c8c8c', margin: '4px 0 0 0' }}>Monitor and manage system activities</p>
        </div>
      ...
      <Modal title={editing ? 'Edit Activity' : 'Log Activity'} open={visible} onOk={onOk} onCancel={() => setVisible(false)} confirmLoading={isSaving}>
