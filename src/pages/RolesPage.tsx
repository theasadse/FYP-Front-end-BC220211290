...
import { ROLES, CREATE_ROLE, UPDATE_ROLE, DELETE_ROLE } from '../graphql/operations/roles'
...
export default function RolesPage() {
  ...
  useEffect(() => { if (rolesData?.roles) setData(rolesData.roles) }, [rolesData])
  ...
  async function onOk() {
    try {
      ...
      const hide = messageApi.loading(editing ? 'Updating role...' : 'Creating role...', 0)
      
      if (editing) {
        await updateRoleMut({ variables: { updateRoleId: editing.id, name: vals.name } })
        hide()
    ...
    <div style={{ padding: '24px' }}>
      ...
      <div style={{ marginBottom: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          ...
          <p style={{ color: '#8c8c8c', margin: '4px 0 0 0' }}>Create, update, and manage user roles</p>
        </div>
      ...
      <Modal title={editing ? 'Edit Role' : 'New Role'} open={visible} onOk={onOk} onCancel={() => setVisible(false)} confirmLoading={isSaving}>
